...
export const socialmediauser = pgTable('socialmediauser', {
    id: integer('id').primaryKey().references(() => appuser.id),
    platform_name: varchar('platform_name', { length: 255 }).references(() => socialmedia.platform_name),
...
export const serviceuser = pgTable('serviceuser', {
    ...
    service_name: varchar('service_name', { length: 255 }).references(() => services.service_name),
});
